      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        >
          ...
          <motion.div
            ...
            className="w-full bg-white/98 backdrop-blur-lg border border-gray-100 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 overflow-hidden flex flex-col md:flex-row max-w-6xl mx-auto"
          >
            {/* Left side: Advisor Image with fixed width on large screens */}
            <div className="relative w-full md:w-1/3 lg:w-80 xl:w-96 h-48 md:h-auto overflow-hidden">
              <div className="absolute inset-0">
                <Image
                  src={
                    advisor.gender === 'Homme'
                  ...
                  className="h-full w-full object-cover object-top" // Object-top keeps face visible
                  width={400}
                  height={400}
                  priority
                />
              ...
              {/* Blurred white separation between image and content (visible only on md+ screens) */}
              <div className="hidden md:block absolute top-0 bottom-0 right-0 w-8 z-10">
                <div className="absolute inset-0 bg-gradient-to-l from-white via-white/90 to-transparent blur-md"></div>
...
>
  ...
  <div className="space-y-4">
    {faqs.map((faq, index) => (
      <div key={index} className="bg-[#ffffff]/90 backdrop-blur-md border border-gray-200 rounded-2xl overflow-hidden">
        <button
        ...
        >
          <p className="text-lg font-medium text-[#213f5b] text-left">
            {faq.question}
          </p>
          {openFAQ === index ? (
            <ChevronUpIcon className="h-6 w-6 text-[#213f5b] flex-shrink-0 ml-4" />
          ) : (
            <ChevronDownIcon className="h-6 w-6 text-[#213f5b] flex-shrink-0 ml-4" />
          )}
        </button>

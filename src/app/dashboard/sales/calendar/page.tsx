...
// Sample events data for Sales (e.g., sales meetings, follow-ups)
const events = [
  {
    id: 1,
    date: "2025-04-05",
    title: "Réunion Vente - Pompe à chaleur - Client X",
    solution: "Pompes a chaleur",
  },
  {
    id: 2,
    date: "2025-04-10",
    title: "Suivi Commercial - Chauffe-eau solaire individuel - Client Y",
    solution: "Chauffe-eau solaire individuel",
  },
  {
    id: 3,
    date: "2025-04-15",
    title: "Présentation Produit - Chauffe-eau thermodynamique - Client Z",
    solution: "Chauffe-eau thermodynamique",
  },
  {
    id: 4,
    date: "2025-04-20",
    title: "Discussion Stratégique - Système Solaire Combiné - Client A",
      ...
      <main className="max-w-7xl mx-auto p-6">
        ...
        >
          ...
          <p className="mt-4 text-lg text-gray-700">
            Visualisez vos réunions, présentations et suivis commerciaux pour nos solutions énergétiques spécialisées.
          </p>
        </motion.div>
            ...
            {calendarDays.map((date, index) => {
              if (!date) {
                return (
                  <div
                    key={index}
                    className="h-16 border border-gray-200 bg-white/50 rounded"
                  ></div>
                );
              }
                  ...
                  {dayEvents.length > 0 && (
                    <div className="absolute bottom-1 left-1 right-1">
                      {dayEvents.map((event, i) => (
                        <span
                          key={i}
                          className="block text-xs text-purple-600 truncate"
                        >
                          {event.title}
                        </span>
                      ))}

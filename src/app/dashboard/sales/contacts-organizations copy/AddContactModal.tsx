...
export function AddContactModal({ onClose }: AddContactModalProps) {
  ...
  // Validate on blur so that errors are displayed as soon as the user leaves the field
  const handleBlur = (name: string, value: string) => {
    const error = validateField(name, value);
    setErrors((prev) => ({ ...prev, [name]: error }));
  };

  // Final validation and submission using the appropriate API endpoint
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Remove keys with empty error messages
    ...
    // For the "Client / Customer (Client Portal)" role, use /api/contacts; otherwise, use /api/users.
    const endpoint =
      role === "Client / Customer (Client Portal)" ? "/api/contacts" : "/api/users";

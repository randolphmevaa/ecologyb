...
export default function MentionsLegalesFactures() {
  // State for each mention-légale field
...
"[Forme juridique] au capital de [montant] euros - [RCS et numéro]"`
  ...
  // Keep a copy of the initial state for the "Réinitialiser" action
  const defaultMentions = { ...mentions };

  // Optional: PDF upload states
  const [pdfFile, setPdfFile] = useState<File | null>(null);
  const [pdfFileName, setPdfFileName] = useState<string>("");

  const fileInputRef = useRef<HTMLInputElement>(null);

  // Handlers for mention text changes
  ...
  const handleCancel = () => {
    ...
    // You can revert to default or navigate away, e.g.:
    // setMentions({ ...defaultMentions });
    // setPdfFile(null);
    // setPdfFileName("");
  };
  ...
  // Enregistrer
  const handleSave = () => {
    // For real usage, call your API or do something else
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-b from-[#f8fafc] to-[#f0f7ff]">
      {/* Top Header (similar to Administration pages) */}
      <Header />
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="my-8">
            <div className="relative">
              ...
              <div className="absolute -z-10 -top-10 -left-10 w-40 h-40 bg-[#bfddf9] opacity-10 rounded-full blur-3xl" />
            </div>
          </div>
          ...
          >
            <div className="mb-4 border-b border-[#eaeaea] pb-2">
              ...
              <p className="text-sm text-[#213f5b] opacity-75">
                Vous pouvez personnaliser les mentions obligatoires de vos factures
              </p>
            </div>

...
interface Contact {
  hasDifferentWorkAddress: boolean | undefined;
  isNewWorkAddress: boolean | undefined;
  civilite: string;
  imageUrl: File | null;
  lastName: string;
  firstName: string;
...
export default function AddContactDossierPage() {
  ...
  // Tab state
  const [activeTab, setActiveTab] = useState("avisImposition");
        ...
        <main className="flex-1 overflow-y-auto p-8">
          ...
          <AnimatePresence>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2 mt-4 flex space-x-4">
                    <Button 
                      ...
                      className="w-full md:w-1/2 bg-[#213f5b] hover:bg-[#5181ad] text-white"
                    >
                ...
                <div className="grid grid-cols-1 gap-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setContact({ ...contact, civilite: e.target.value })}
                      onBlur={(e) => handleContactBlur("civilite", e.target.value)}
                      className="mt-1 block w-full rounded-md border border-gray-300 bg-white shadow-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                      <option value="">Sélectionnez</option>
                    ...
                    {contactErrors.civilite && <p className="mt-1 text-xs text-red-500">{contactErrors.civilite}</p>}
                  </div>
                  {/* Nom & Prénom */}
                  ...
                  {/* Adresse de travaux différente ? Checkbox */}
                  <div className="mt-4">
                    <div className="flex items-center">
                      <input 
                        type="checkbox" 
                        id="differentWorkAddress" 
                        checked={contact.hasDifferentWorkAddress}
                        onChange={(e) => {
                          const isChecked = e.target.checked;
                          setContact(prev => ({
                            ...prev, 
                            hasDifferentWorkAddress: isChecked,
                            // Reset work address fields if unchecked
                            ...(isChecked ? {} : {
                              workAddress: '',
                              workDepartment: '',
                              workPostalCode: '',
                      ...
                      />
                      <label htmlFor="differentWorkAddress" className="ml-2 block text-sm text-gray-900">
                        Adresse de travaux différente ?
                      </label>
                    </div>

                    {/* Additional Work Address Section */}
                    {contact.hasDifferentWorkAddress && (
                      <div className="mt-4 space-y-4">
                        {/* Nouvelle adresse ? Checkbox */}
                        <div className="flex items-center">
                          <input 
                            type="checkbox" 
                            id="newWorkAddress" 
                            checked={contact.isNewWorkAddress}
                            onChange={(e) => {
                              const isChecked = e.target.checked;
                              setContact(prev => ({
                                ...prev, 
                                isNewWorkAddress: isChecked,
                                // Optional: Add logic to pre-fill or reset fields based on checkbox
                              }));
                            }}
                            className="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded"
                          />
                          <label htmlFor="newWorkAddress" className="ml-2 block text-sm text-gray-900">
                            Nouvelle adresse ? <span className="text-xs text-gray-500">(Ne pas cocher s&apos;il s&apos;agit d&apos;une habitation secondaire.)</span>
                          </label>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {/* Adresse des travaux */}
                          <div>
                            <label htmlFor="workAddress" className="block text-sm font-medium text-gray-700">Adresse des travaux</label>
                            <input 
                              type="text" 
                              id="workAddress" 
                          ...
                          <div>
                            <label htmlFor="workDepartment" className="block text-sm font-medium text-gray-700">Département*</label>
                            <select
                              id="workDepartment"
                              value={contact.workDepartment || ''}
                              onChange={(e) => setContact(prev => ({
                                ...prev, 
                                workDepartment: e.target.value
                              }))}
                              className="mt-1 block w-full rounded-md border border-gray-300 bg-white shadow-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                            >
                              <option value="">Sélectionnez</option>
                          ...
                          <div>
                            <label htmlFor="workPostalCode" className="block text-sm font-medium text-gray-700">Code postal des travaux</label>
                            <input 
                              type="text" 
                              id="workPostalCode" 
                              value={contact.workPostalCode || ''} 
                              onChange={(e) => setContact(prev => ({
                                ...prev, 
                                workPostalCode: e.target.value
                              }))} 
                          ...
                          {/* Ville des travaux */}
                          <div>
                            <label htmlFor="workCity" className="block text-sm font-medium text-gray-700">Ville des travaux</label>
                            <input 
                              type="text" 
                              id="workCity" 
                              value={contact.workCity || ''} 
                              onChange={(e) => setContact(prev => ({
                                ...prev, 
                          ...
                          {/* Pays des travaux */}
                          <div>
                            <label htmlFor="workCountry" className="block text-sm font-medium text-gray-700">Pays des travaux</label>
                            <input 
                              type="text" 
                              id="workCountry" 
                              value={contact.workCountry || ''} 
                              onChange={(e) => setContact(prev => ({
                                ...prev, 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="mt-2 flex">
                      ...
                      <Button type="button" variant="primary" onClick={addComment} className="bg-[#213f5b] hover:bg-[#5181ad] text-white">Ajouter</Button>
                    </div>
                  </div>
                </div>
              </motion.div>
                    ...
                    <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                      {avisModalStep === 1 ? (
                        <Button 
                          ...
                          className="bg-[#213f5b] hover:bg-[#5181ad] text-white"
                        >
                          Suivant
                        </Button>
                      ) : (
            ...
            <div className="mt-8 flex justify-end space-x-4">
              ...
              {activeTab !== "projet" ? (
                <Button variant="primary" onClick={goToNextTab} disabled={isSubmitting} className="bg-[#213f5b] hover:bg-[#5181ad] text-white">Suivant</Button>
              ) : (
